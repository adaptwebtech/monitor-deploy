...
**Comportamento pós-fix 2026-05-22 (ws-completion):**
- `isMounted` flag: `onMounted` define `isMounted = true` somente após todos os `await` concluírem. O watcher `watch(dateRange)` faz early-return enquanto `!isMounted`, eliminando a race condition em que a chamada fire-and-forget do watcher sobrescrevia `pipelines.value` depois de um evento WS já ter sido aplicado.
- `onReconnect` registrado: `socketConnection.onReconnect(() => dashboardStore.fetchInitial())` — ao reconectar (ex.: pod API restartado pelo force-deploy), o dashboard refaz o fetch inicial do banco e garante estado consistente.
- Token fallback: `authStore.accessToken ?? ""` — evita passar `null` para o composable se o token ainda não estiver disponível.
...
| `dashboard.store.ts` | `pipelines`, `kpis`, `loading`, `error`, `dateStart`, `dateEnd`, `runningPipeline` (computed) | `fetchPipelines()`, `fetchKpis()`, `setDateRange()`, `handleSocketCreated()`, `handleSocketUpdated()` · **pós-fix 2026-05-22:** `handleSocketUpdated` é `async`; chama `fetchKpis(dateStart, dateEnd)` quando `pipeline.status === 'Completed'` ou `'Failed'`; upsert no topo da lista (`pipelines.value = [pipeline, ...pipelines.value]`) quando `idx === -1` em vez de descartar silenciosamente |
...
```ts
const { onCreated, onUpdated, disconnect, onReconnect } = usePipelineSocket(accessToken)
```

Conecta ao namespace `window.config.WS_URL + "/pipeline"` com `auth: { token }` via `socket.io-client`. Expõe callbacks por evento.

- `onReconnect(cb)` — registra callback a ser disparado no evento `connect` do socket (inclui reconexões após queda). Internamente mantém um array `reconnectCallbacks` e um handler `socket.on('connect', ...)` que itera e executa cada callback. Permite que o chamador reaja a restabelecimentos de conexão (ex.: pod restart) sem polling. **Adicionado em 2026-05-22 (ws-completion).**
...
`usePipelineSocket` não implementa reconexão automática por tentativas (banner "Conexão perdida" da spec ainda não implementado). Porém, a partir de **2026-05-22 (ws-completion)**, o composable expõe `onReconnect(cb)` — ao `socket.io-client` reestabelecer a conexão por conta própria (comportamento padrão do cliente), o callback registrado é disparado. `DashboardView` utiliza isso para chamar `fetchInitial()` e resincronizar com o banco após quedas de conexão (ex.: pod restart pelo Argo `force-deploy`).
...
- **2026-05-22** — Hotfix lint (branch: hotfix, slug: lint-hotfix): `DashboardView.reg.spec.ts` possuía dois erros de lint bloqueantes — `require()` dentro de `mockApiFetchSuccess` (violando `@typescript-eslint/no-var-requires`) e `// @ts-ignore` sem descrição (violando `@typescript-eslint/ban-ts-comment` configurado como `allow-with-description`). Função morta `mockApiFetchSuccess` removida; descrição `mock compatibility` adicionada ao `@ts-ignore` restante. Lint GREEN. Ver triage: `docs/fixes/pipeline-monitor-lint-hotfix.md`.

- **2026-05-22** — Fix WebSocket completion (branch: simple-fix, slug: ws-completion): tabela do dashboard não atualizava quando pipeline completava via `notify-exit` após pod restart pelo Argo `force-deploy`. `usePipelineSocket` agora expõe `onReconnect(cb)` (array `reconnectCallbacks` + `socket.on('connect', ...)`); `DashboardView` registra `onReconnect(() => dashboardStore.fetchInitial())` e adiciona `isMounted` flag para proteger o watcher `dateRange`; `dashboard.store.ts` `handleSocketUpdated` passa a ser `async`, chama `fetchKpis` em `Completed`/`Failed`, e faz upsert no topo quando `idx === -1`. REG-1..5 GREEN. Ver triage: `docs/fixes/pipeline-monitor-ws-completion.md`.
