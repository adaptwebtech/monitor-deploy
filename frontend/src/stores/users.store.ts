...
export const useUsersStore = defineStore('users', () => {
  ...
  async function fetchUsers(
    params: { search?: string; del?: string; page?: number; limit?: number } = {},
  ) {
    loading.value = true
    error.value = null
    try {
      ...
      const res = await fetch(`${window.config.API_URL}/users?${q}`, { headers: getHeaders() })
  ...
  async function regenerateToken(id: string) {
    const res = await fetch(`${window.config.API_URL}/users/${id}/regenerate-token`, {
      method: 'POST',
      headers: getHeaders(),
    })
    if (!res.ok) throw new Error(`HTTP ${res.status}`)
  ...
  return { users, total, page, limit, loading, error, fetchUsers, updateUser, deleteUser, regenerateToken }
})
