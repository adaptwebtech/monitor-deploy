...
describe('LoginView', () => {
  ...
  function mountLoginView(loginImpl?: () => Promise<void>) {
  ...
  it('left image has Bootstrap classes that hide it on mobile (d-none d-md-block)', () => {
    ...
    const hasHiddenOnMobile =
      ...
      loginImage.element.classList.contains('d-none') ||
      loginImage.element.closest('.d-none') !== null

    // The image container must carry d-none so it is hidden on xs/sm
    expect(
      classes.includes('d-none') || loginImage.attributes('class')?.includes('d-none'),
    ).toBe(true)
