...
async function handleSearch(event: Event) {
  ...
  search.value = value
  await usersStore.fetchUsers({ search: value, del: delFilter.value, page: 1, limit: 10 })
}

async function handleDelFilterChange(event: Event) {
  const value = (event.target as HTMLSelectElement).value
  delFilter.value = value
  await usersStore.fetchUsers({ search: search.value, del: value, page: 1, limit: 10 })
}
</script>
...
<template>
  <AppLayout>
    <div class="container py-4">
      ...
      <div v-else-if="usersStore.error" class="alert alert-danger">{{ usersStore.error }}</div>
      <div v-else>
        <table class="table table-hover align-middle" data-test="users-table">
          ...
          <tbody>
            ...
            >
              <td>
                <div
                  style="width: 36px; height: 36px; border-radius: 50%; overflow: hidden; display: inline-flex; align-items: center; justify-content: center; background: #6c757d; color: white; font-size: 14px;"
                >
                  <img
            ...
            <tr v-if="usersStore.users.length === 0">
              <td :colspan="isRoot ? 5 : 4" class="text-center text-muted py-4" data-test="empty-state">
